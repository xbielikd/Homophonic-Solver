...
public class HomophonicAnnealingSolver {
    ...
    public SolverResult solve() {
        ...
        for (int r = 0; r < NUM_RESTARTS; r++) {
            ...
            score = exhaustiveReassignPass(key, pt, score);  // ← add this
    ...
    private double exhaustiveReassignPass(int[] key, int[] pt, double score) {
        boolean improved;
        do {
            improved = false;
            for (int c = 0; c < N; c++) {
                int oldLetter = key[c];
                double bestScore = score;
                int bestLetter = oldLetter;

                for (int letter = 0; letter < 26; letter++) {
                    if (letter == oldLetter) continue;
                    if (countForLetter(key, oldLetter) <= 1
                            && letterHasCiphertextPositions(key, oldLetter)) continue;

                    // try c → letter using delta scoring
                    int n = collectAffectedSingle(c);
                    double oldSum = 0.0;
                    for (int i = 0; i < n; i++) oldSum += quad.scoreAt(pt, affected[i]);
...
//        if (oldLetter == newLetter) return score;
//
//        if (countForLetter(key, oldLetter) <= 1) return score;
//
//        int n = collectAffectedSingle(c);
//
//        double oldSum = 0.0;
...
//                for (int letter = 0; letter < 26; letter++) {
//                    if (letter == oldLetter) continue;
//                    // don't strip any letter of its last symbol
//                    if (countForLetter(key, oldLetter) <= 1) continue;
//
//                    // try reassigning c -> letter
//                    key[c] = letter;
//                    for (int pos : prep.positionsOfCipher[c]) pt[pos] = letter;
